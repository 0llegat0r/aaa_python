...
class MyCountVectorizer:
    def __init__(self, vocabulary: Dict[str, int] = None) -> None:
        if vocabulary is not None:
            self._vocabulary = {key: value for key, value in sorted(vocabulary.items(), key=lambda x: x[1])}
    ...
    def _validate_vocabulary_type(self) -> None:
        ...
        if set(range(count)) != set(self._vocabulary.values()):
            raise ValueError("vocabulary values don't from a sequence from 0 to {}".format(count))
...
def test_default() -> None:
    ...
    assert feature_names == ['crock', 'pot', 'pasta', 'never',
                             'boil', 'again', 'pomodoro', 'fresh',
                             'ingredients', 'parmesan', 'to', 'taste']
    assert x == [[1, 1, 2, 1, 1, 1, 0, 0, 0, 0, 0, 0],
